...
"Antes de confirmar cualquier plan te indicaremos por escrito el precio, el método de pago y las condiciones de cancelación aplicables. No tendrás que realizar ningún pago hasta conocer y aceptar previamente esas condiciones.",
],
[
...
if (mode === "whatsapp" && /^\d{8,15}$/.test(config.contact.whatsapp)) {
window.location.href = `https://wa.me/${config.contact.whatsapp}?text=${encodeURIComponent(
      proposal,
    )}`;
...
$("#form-status").textContent =
"Propuesta preparada. No se ha enviado ningún mensaje.";
